...
class cm_completion_details {
    ...
    public function get_details(): array {
        ...
        if (!is_null($this->cminfo->completiongradeitemnumber)) {
			...
			global $DB;
            $latestGrade = $DB->get_record_sql('SELECT grade FROM {quiz_grades} qg
					WHERE quiz = (SELECT cm.instance FROM {course_modules} cm WHERE cm.id = ' . $this->cminfo->id . ') 
					 AND qg.userid = ' . $this->userid);
			if ($completiondata->completiongrade == 1) {
				/*$latestGrade = $DB->get_record_sql('SELECT grade FROM {quiz_grades} qg
					WHERE quiz = (SELECT cm.instance FROM {course_modules} cm WHERE cm.id = ' . $this->cminfo->id . ') 
					 AND qg.userid = ' . $this->userid .
					' ORDER BY qg.timemodified DESC LIMIT 1');*/

				$details['completionusegrade'] = (object)[
					'status' => $status,
					'description' => ($status <> 0 and $status <> 4) ? round((float) $latestGrade->grade, 2) : '',//get_string('detail_desc:receivegrade', 'completion'),
				];
			} else {
				$details['completionusegrade'] = (object)[
					'status' => $status,
            ...
            if (!is_null($this->cminfo->completionpassgrade) && $this->cminfo->completionpassgrade) {
                $details['completionpassgrade'] = (object)[
                    ...
                    'description' => 'xxxx ' . get_string('detail_desc:receivepassgrade', 'completion'),
                ];
            }
        }
