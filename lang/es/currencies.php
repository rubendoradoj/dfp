<?php
// This file is part of Moodle - https://moodle.org/
//
// Moodle is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// Moodle is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License
// along with Moodle.  If not, see <https://www.gnu.org/licenses/>.

/**
 * Strings for component 'currencies', language 'es', version '4.2'.
 *
 * @package     currencies
 * @category    string
 * @copyright   1999 Martin Dougiamas and contributors
 * @license     https://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

defined('MOODLE_INTERNAL') || die();

$string['AED'] = 'Dirham de los Emiratos Arabes Unidos';
$string['AFN'] = 'Afgani Afgano';
$string['ALL'] = 'Lek Albano';
$string['AMD'] = 'Dram Armenio';
$string['ANG'] = 'Antillas Holandesas';
$string['AOA'] = 'Kwanza Angoleño';
$string['ARS'] = 'Peso argentino';
$string['AUD'] = 'Dólar Australiano';
$string['AWG'] = 'Guilder de Aruba';
$string['AZN'] = 'Manat Azerbaiyano';
$string['BAM'] = 'Marco Convertible de Bosnia y Herzegovina';
$string['BBD'] = 'Dólar de Barbados';
$string['BDT'] = 'Taka de Bangladesh';
$string['BGN'] = 'Lev Búlgaro';
$string['BHD'] = 'Dinar Bahreiní';
$string['BIF'] = 'Franco burundés';
$string['BMD'] = 'Dólar de Bermuda';
$string['BND'] = 'Dólar de Brunei';
$string['BOB'] = 'Boliviano de Bolivia';
$string['BRL'] = 'Real brasileño';
$string['BSD'] = 'Dólar Bahameño';
$string['BTN'] = 'Bután';
$string['BWP'] = 'Botswana';
$string['BYR'] = 'Rublo BieloRuso';
$string['BZD'] = 'Dólar de Belice';
$string['CAD'] = 'Dólar Canadiense';
$string['CDF'] = 'Franco Congoleño';
$string['CHF'] = 'Franco Suizo';
$string['CLF'] = 'Unidades de Fomento Chilenas';
$string['CLP'] = 'Peso Chileno';
$string['CNY'] = 'Yuan Chino (Renminbi)';
$string['COP'] = 'Peso Colombiano';
$string['CRC'] = 'Colón Costarricense';
$string['CUC'] = 'Peso Convertible cubano';
$string['CUP'] = 'Peso Cubano';
$string['CVE'] = 'Escudo caboverdiano';
$string['CZK'] = 'Corona/Koruna Checa';
$string['DJF'] = 'Franco Yibutiano';
$string['DKK'] = 'Corona Danesa';
$string['DOP'] = 'Peso Dominicano';
$string['DZD'] = 'Dinar Algerino';
$string['EGP'] = 'Libra Egipcia';
$string['ERN'] = 'Nakfa Eritreo';
$string['ETB'] = 'Birr Etíope';
$string['EUR'] = 'Euro';
$string['FJD'] = 'Dólar Fijiano';
$string['FKP'] = 'Libra Malvinense';
$string['GBP'] = 'Libra esterlina';
$string['GEL'] = 'Lari Georgiano';
$string['GHS'] = 'Cedi de Ghana';
$string['GIP'] = 'Libra de Gibraltar';
$string['GMD'] = 'Dalasi Gambiano';
$string['GNF'] = 'Franco Guineano';
$string['GTQ'] = 'Quetzal Guatemalteco';
$string['GYD'] = 'Dólar guyanés';
$string['HKD'] = 'Dólar de Hong Kong';
$string['HNL'] = 'Lempira Hondureña';
$string['HRK'] = 'Kuna Croata';
$string['HTG'] = 'Gourde Haitiano';
$string['HUF'] = 'Florín/Forint Húngaro';
$string['IDR'] = 'Rupiah Indonesa';
$string['ILS'] = 'Scheckel Israelí';
$string['INR'] = 'Rupia India';
$string['IQD'] = 'Dinar Iraquí';
$string['IRR'] = 'Rial Iraní';
$string['ISK'] = 'Corona/Króna Islandesa';
$string['JMD'] = 'Dólar Jamaicano';
$string['JOD'] = 'Dinar Jordano';
$string['JPY'] = 'Yen Japonés';
$string['KES'] = 'Chelín keniano';
$string['KGS'] = 'Som kirguís';
$string['KHR'] = 'Riel Camboyano';
$string['KMF'] = 'Franco de Comoras';
$string['KPW'] = 'Won Norcoreano';
$string['KRW'] = 'Won Surcoreano';
$string['KWD'] = 'Dinar Kuwaití';
$string['KYD'] = 'Dólar de la Islas Caimán';
$string['KZT'] = 'Tenge Kazajo';
$string['LAK'] = 'Kip Laosiano';
$string['LBP'] = 'Libra Libanesa';
$string['LKR'] = 'Rupia de Sri Lanka';
$string['LRD'] = 'Dólar Liberiano';
$string['LSL'] = 'Loti Lesothense';
$string['LTL'] = 'Litas Lituana';
$string['LVL'] = 'Lats Letón';
$string['LYD'] = 'Dinar Libio';
$string['MAD'] = 'Dirham Marroquí';
$string['MDL'] = 'Leu Moldavo';
$string['MGA'] = 'Ariary Malgache';
$string['MKD'] = 'Denar Macedonio';
$string['MMK'] = 'Kyat Birmano (Myanmar)';
$string['MNT'] = 'Tughrik Mongol';
$string['MOP'] = 'Pataca de Macao';
$string['MRO'] = 'Ouguiya Mauritana';
$string['MUR'] = 'Rupia Mauricia';
$string['MVR'] = 'Rupia de Maldivas';
$string['MWK'] = 'Kwacha Malawiano';
$string['MXN'] = 'Peso Mexicano';
$string['MYR'] = 'Ringgit Malayo';
$string['MZN'] = 'Metical Mozambiqueño';
$string['NAD'] = 'Dólar Namibio';
$string['NGN'] = 'Naira Nigeriana';
$string['NIO'] = 'Córdoba Nicaragüense Oro';
$string['NOK'] = 'Kroner Noruega';
$string['NPR'] = 'Rupia Nepalesa';
$string['NZD'] = 'Dólar Neozelandés';
$string['OMR'] = 'Rial Omaní';
$string['PAB'] = 'Balboa Panameña';
$string['PEN'] = 'Nuevo sol peruano';
$string['PGK'] = 'Kina de Papúa Nueva Guinea';
$string['PHP'] = 'Peso Filipino';
$string['PKR'] = 'Rupia Pakistaní';
$string['PLN'] = 'Esloti polaco';
$string['PYG'] = 'Guaraní paraguayo';
$string['QAR'] = 'Rial Qatarí';
$string['RON'] = 'Leu Rumano';
$string['RSD'] = 'Dinar Serbio';
$string['RUB'] = 'Rublo ruso';
$string['RWF'] = 'Franco Ruandés';
$string['SAR'] = 'Riyal Saudí';
$string['SBD'] = 'Dólar de las Islas Salomón';
$string['SCR'] = 'Rupia de Seychelles';
$string['SDG'] = 'Libra Sudanesa';
$string['SEK'] = 'Corona/Krona Sueca';
$string['SGD'] = 'Dólar de Singapur';
$string['SHP'] = 'Libra de Santa Helena';
$string['SLL'] = 'Leone de Sierra Leona';
$string['SOS'] = 'Chelín somalí';
$string['SRD'] = 'Dólar de Surinam';
$string['SSP'] = 'Libra SurSudanesa';
$string['STD'] = 'Dobra de Santo Tomé y Príncipe';
$string['SVC'] = 'Colón Salvadoreño';
$string['SYP'] = 'Libra siria';
$string['SZL'] = 'Lilangeni suazi';
$string['THB'] = 'Baht tailandés';
$string['TJS'] = 'Somoni tayiko';
$string['TMT'] = 'Manat turkmeno';
$string['TND'] = 'Dinar Tunecino';
$string['TOP'] = 'Pa`anga Tongano';
$string['TRY'] = 'Lira Turca';
$string['TTD'] = 'Dólar de Trinidad y Tobago';
$string['TWD'] = 'Dólar Taiwanés';
$string['TZS'] = 'Chelín tanzano';
$string['UAH'] = 'Grivna Ucraniana';
$string['UGX'] = 'Chelín Ugandés';
$string['USD'] = 'Dólar Estadounidense';
$string['UYU'] = 'Peso uruguayo';
$string['UZS'] = 'Som uzbeko';
$string['VEF'] = 'Bolívar Fuerte venezolano';
$string['VND'] = 'Dong Vietnamita';
$string['VUV'] = 'Vatu vanuatense';
$string['WST'] = 'Tala Samoana';
$string['XAF'] = 'Franco CFA de África Central';
$string['XCD'] = 'Dólar del Caribe del este';
$string['XOF'] = 'Franco CFA de África Occidental';
$string['XPF'] = 'Franco CFP';
$string['YER'] = 'Rial yemení';
$string['ZAR'] = 'Rand Sudafricano';
$string['ZMW'] = 'Kwacha Zambiano';
$string['ZWL'] = 'Dólar Zimbabuense';
