...
class renderer extends plugin_renderer_base {
    ...
    public function cancel_quiz_attempt_link(quiz_attempt $attemptobj) {
        $url = $attemptobj->view_url();
       ...
       /* if ($attemptobj->get_access_manager(time())->attempt_must_be_in_popup()) {
            $this->page->requires->js_init_call('M.mod_quiz.secure_window.init_close_button',
                [$url->out(false)], false, quiz_get_js_module());
            return html_writer::empty_tag('input', ['type' => 'button',
                'value' => 'Cancelar intento1',
                'id' => 'secureclosebuttonCancelAttempt',
                'class' => 'mod_quiz-next-nav btn btn-primary']);

        } else {*/
        global $CFG;
        $url = new moodle_url( $CFG->wwwroot . '/mod/quiz/cancel_quiz_attempt.php?id=' .
            $attemptobj->get_courseid() . '&cancelQuizAttempt=' .
            $attemptobj->get_attemptid() . '&quizId=' .
            $attemptobj->get_quizid());

        return html_writer::link($url, 'Cancelar intento');
            //,    ['class' => 'mod_quiz-next-nav']);
        //}
    }
