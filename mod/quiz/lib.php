...
function mod_quiz_cm_info_dynamic(cm_info $cm) {
    ...
    $miquizid =  $DB->get_record_sql('SELECT cm.instance as miid FROM {course_modules} cm WHERE cm.id = ' . ((string) $cm->id));
     $numpreguntasquiz = $DB->get_record_sql('SELECT COUNT(*) as numquestions FROM {quiz_slots} qs WHERE qs.quizid = ' . ((string) $miquizid->miid));
    $intentosUsuarioActual = quiz_get_user_attempts(((string) $miquizid->miid), $USER->id);


    $attempts = quiz_get_all_users_attempts(((string) $miquizid->miid));
    $notaPromedio = 0;
    if ((int) $numpreguntasquiz->numquestions > 0 and count($attempts) > 0) {
        foreach ($attempts as &$value) {
            $notaPromedio += (float)$value->sumgrades / (float)$numpreguntasquiz->numquestions;
        }

        $notaPromedio = ((float)$notaPromedio * 10) / count($attempts);
    }


    global $PAGE;
	//$miquiz = $DB->get_record('quiz', ['id' => $cm->id]);
    $quizobj = quiz_settings::create($miquizid->miid, $USER->id);
    $quiz = $quizobj->get_quiz();

	 $out = '<span class="badge badge-secondary" style="border-radius:5px">' . (string) $numpreguntasquiz->numquestions . ' preguntas, ' . count($intentosUsuarioActual) . ' veces realizado</span>';
    if ($quiz->timelimit > 0)
        $out.= '<span class="badge badge-secondary" style="border-radius:5px" title="Duración del cuestionario">' . $quiz->timelimit / 60 . ' minutos</span>';

	 $out.= '<span class="badge badge-secondary" style="border-radius:5px"><a href="' . $cm->url . '">Últimos intentos</a></span>';
	 $out.= '<span class="badge badge-primary" style="background-color:blue;color:white;border-radius:5px" title="Nota media">Media DFPOL: ' . round((float) $notaPromedio, 2) . '</span>';
	 $out.= '<span class="badge badge-secondary" style="border-radius:5px"><a href="' . $quizobj->start_attempt_url() . '">Empezar test</a></span>';

/*
    //El siguiente codigo fue tomado de mod/quiz/view.php
    $quizobj = quiz_settings::create($miquizid->miid, $USER->id);
    $timenow = time();
    $quiz = $quizobj->get_quiz();
    //$cm = $quizobj->get_cm();
    ...
    $viewobj->buttontext = 'INICIAR';*/

    //$PAGE->get_renderer('mod_quiz')->view_page_tertiary_nav($viewobj);
    //$PAGE->get_renderer('mod_quiz')->start_attempt_button('textoboton', $cm->url);
            //$quiz, $cm, $context, $returnzero, $currentgroup);
    //print_r($viewobj);
	 $cm->set_after_link($out);
}

function quiz_get_all_users_attempts($quizids, $status = 'finished', $includepreviews = false) {
    global $DB;
    ...
    return $DB->get_records_select('quiz_attempts',
        "quiz $insql AND 1 = 1" . $previewclause . $statuscondition,
