...
<script>
    ...
    function createDiv(){
        const divWrapperTime = document.createElement("div");
        divWrapperTime.id = "quiz-timer-wrapper";
        divWrapperTime.setAttribute("class", "mb-2");
        divWrapperTime.style.display = "flex";
        return divWrapperTime;
    }

    function reubicateTimer(timer){
        newDiv = createDiv();
        newDiv.appendChild(timer, newDiv);
        const newTimer = timer.cloneNode(true);
        const divMainContent = document.getElementsByClassName("main-content")[0];
        divMainContent.parentNode.insertBefore(newDiv, divMainContent);
        
    }

    function clickFinish(){
        const formSection = document.getElementById("frm-finishattempt");
        for (let element of formSection){
            if(element.type === "submit"){
                setTimeout(() => {
                    const timer = document.getElementById("quiz-timer");
                    reubicateTimer(timer);
