...
class theme_spacechild_core_tag_renderer extends plugin_renderer_base {
    ...
    public function tag_search_page($query = '', $tagcollid = 0) {
        ...
        if ($tagcloud->get_count()) {
            $searchresults = $this->output->render_from_template(
                'core_tag/tagcloud',
    ...
    public function tag_index_page($tag, $entities, $tagareaid, $exclusivemode, $fromctx, $ctx, $rec, $page) {
        ...
        $taglist = new \core_tag\output\taglist(
            $relatedtags,
            get_string('relatedtags', 'tag'),
            'tag-relatedtags',
            $relatedtagslimit
        );
        $rv .= $this->output->render_from_template(
            'core_tag/taglist',
        ...
        if (count($entitylinks) > 1) {
            $rv .= '<div class="tag-index-toc mt-2 mb-4 rui-tabs"><ul class="nav nav-tabs">' .
            join('', $entitylinks) . '</ul></div>';
        } else if (!$entitylinks) {
            $rv .= '<div class="tag-noresults alert alert-danger">' .
    ...
    protected function tag_links($tag) {
        if ($links = $tag->get_links()) {
            $content = '<ul class="nav nav-tabs rui-tag-tabs" role="tablist"><li class="nav-item">' .
            implode('</li> <li class="nav-item">', $links) . '</li></ul>';
            return html_writer::div($content, 'tag-management-box rui-tabs mb-4');
        }
        return '';
    }
    ...
    protected function search_form($query = '', $tagcollid = 0) {
        ...
        $output = '<div class="simplesearchform d-flex justify-content-center">
        <form action="' . $searchurl . '" class="mform form-inline w-100">';
        $output .= '<div class="search-input-group d-inline-flex justify-content-between w-100">
        <label class="accesshide" for="searchform_query">' . get_string('searchtags', 'tag') . '</label>';
        ...
        $output .= '<input id="searchform_query" class="search-input w-100" name="query" type="text" size="40" value="'
                    . s($query) .
                    '" />';
