...
// Hidden sidebar.
if (theme_spacechild_get_setting('turnoffsidebarfp') == '1' || !isloggedin()) {
    $hiddensidebar = true;
    $navdraweropen = false;
...
$total = 23;
for ($i = 0; $i <= $total; $i++) {
    ${"block" . $i} = theme_spacechild_get_setting("block" . $i);
}

$array = array();

for ($i = 0; $i <= $total; $i++) {
    if (theme_spacechild_get_setting("displayblock" . $i) == '1') {
        $array[$i] = ${"block" . $i};
    }
    $array[0] = $block0;
}
...
echo $OUTPUT->render_from_template('theme_spacechild/tmpl-frontpage', $templatecontext);
