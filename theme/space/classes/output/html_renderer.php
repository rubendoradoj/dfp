...
class html_renderer extends \plugin_renderer_base {
    ...
    protected function image_header($fixednavbar = false) {
        ...
        foreach ($images as $image) {
            if (!empty($settings->$image)) {
                ...
                if ($CFG->branch >= 33) {
                    $template->$image = $this->image_url($image, 'theme_space');
                } else {
                    $template->$image = $this->pix_url($image, 'theme_space');
                }
            }
        }
        ...
        return $this->render_from_template('theme_space/imageheading', $template);
    }
    ...
    protected function navigation_menu($fixednavbar = false) {
        ...
        return $this->render_from_template('theme_space/navigation', $template);
    }

    /**
     * Render the social icons shown in the page footer.
     */
    public function space_socialicons() {
        global $CFG;
        $content = '';
        ...
        return $this->render_from_template('theme_space/socialicons', $template);
    }
    ...
    public function languagemenu() {
        ...
        if ($haslangmenu) {
            ...
            return $this->render_from_template('theme_space/language', $langmenu);
        }
    }
    ...
    public function footer() {
        ...
        $template->socialicons = $this->space_socialicons();
        ...
        return $this->render_from_template('theme_space/footer', $template);
    }
