...
function theme_space_css_tree_post_processor($tree, $theme) {
    debugging('theme_space_css_tree_post_processor() is deprecated. Required' .
...
function theme_space_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    if ($context->contextlevel == CONTEXT_SYSTEM) {
        ...
        if ($filearea === 'hvp') {
            return theme_space_serve_hvp_css($args[1], $theme);
        }
        if ($filearea === 'favicon') {
...
function theme_space_get_course_information_banners() {
    ...
    if (get_config('theme_space', 'showhintcoursehidden') == 1
            && $allowtoshowhint == true
            && $PAGE->has_set_url()
        ...
        $html .= $OUTPUT->render_from_template('theme_space/course-hint-hidden', $templatecontext);
    }
    ...
    if (get_config('theme_space', 'showhintcourseguestaccess') == 1
        ...
        // Render template and add it to HTML code.
        $html .= $OUTPUT->render_from_template('theme_space/course-hint-guestaccess', $templatecontext);
    }
    ...
    if (get_config('theme_space', 'showhintcourseselfenrol') == 1
            && $allowtoshowhint == true
            && $PAGE->has_set_url()
            && $PAGE->url->compare(new moodle_url('/course/view.php'), URL_MATCH_BASE)
            && $COURSE->visible == true) {
        ...
        if (!empty($selfenrolinstances) &&
            ...
            if ($selfenrolmentpossiblecurrently == true) {
                $templatecontext['selfenrolhintstart'] = get_string('showhintcourseselfenrolstartcurrently', 'theme_space');
            } else if ($selfenrolmentpossiblefuture == true) {
                $templatecontext['selfenrolhintstart'] = get_string('showhintcourseselfenrolstartfuture', 'theme_space');
            }
            ...
            foreach ($selfenrolinstances as $selfenrolinstanceid => $selfenrolinstanceobject) {
                ...
                if ($selfenrolinstanceobject->unrestrictedness == 'unlimited') {
                    $templatecontext['selfenrolinstances'][] = get_string('showhintcourseselfenrolunlimited', 'theme_space',
                            array('name' => $selfenrolinstanceobject->name));
                } else if ($selfenrolinstanceobject->unrestrictedness == 'until') {
                    $templatecontext['selfenrolinstances'][] = get_string('showhintcourseselfenroluntil', 'theme_space',
                            array('name' => $selfenrolinstanceobject->name,
                                    'until' => userdate($selfenrolinstanceobject->enddate)));
                } else if ($selfenrolinstanceobject->unrestrictedness == 'from') {
                    $templatecontext['selfenrolinstances'][] = get_string('showhintcourseselfenrolfrom', 'theme_space',
                            array('name' => $selfenrolinstanceobject->name,
                                    'from' => userdate($selfenrolinstanceobject->startdate)));
                } else if ($selfenrolinstanceobject->unrestrictedness == 'since') {
                    $templatecontext['selfenrolinstances'][] = get_string('showhintcourseselfenrolsince', 'theme_space',
                            array('name' => $selfenrolinstanceobject->name,
                                    'since' => userdate($selfenrolinstanceobject->startdate)));
                } else if ($selfenrolinstanceobject->unrestrictedness == 'fromuntil') {
                    $templatecontext['selfenrolinstances'][] = get_string('showhintcourseselfenrolfromuntil', 'theme_space',
                            array('name' => $selfenrolinstanceobject->name,
                                    'until' => userdate($selfenrolinstanceobject->enddate),
                                    'from' => userdate($selfenrolinstanceobject->startdate)));
                } else if ($selfenrolinstanceobject->unrestrictedness == 'sinceuntil') {
                    $templatecontext['selfenrolinstances'][] = get_string('showhintcourseselfenrolsinceuntil', 'theme_space',
                            array('name' => $selfenrolinstanceobject->name,
                                    'until' => userdate($selfenrolinstanceobject->enddate),
                                    'since' => userdate($selfenrolinstanceobject->startdate)));
                }
            ...
            // Render template and add it to HTML code.
            $html .= $OUTPUT->render_from_template('theme_space/course-hint-selfenrol', $templatecontext);
        }
    }
    ...
    if (get_config('theme_space', 'showswitchedroleincourse') == 1
            && is_role_switched($COURSE->id) ) {
        ...
        // Render template and add it to HTML code.
        $html .= $OUTPUT->render_from_template('theme_space/course-hint-switchedrole', $templatecontext);
    }
 ...
 * @throws dml_exception
 */
function theme_space_serve_hvp_css($filename, $theme) {
    global $CFG, $PAGE;
    ...
    $settings = new \theme_space\util\theme_settings();
    $content = $settings->hvpcss;

    $md5content = md5($content);
    $md5stored = get_config('theme_space', 'hvpccssmd5');
