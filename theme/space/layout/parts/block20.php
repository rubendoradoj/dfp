...
$block20wrapperalign = theme_space_get_setting('block20wrapperalign');

$imgslidesonly = theme_space_get_setting('imgslidesonly');
$sliderloop = theme_space_get_setting('sliderloop');
$sliderintervalenabled = theme_space_get_setting('sliderintervalenabled');
$sliderinterval = theme_space_get_setting('sliderinterval');
$sliderclickable = theme_space_get_setting('sliderclickable');
$rtlslider = theme_space_get_setting('rtlslider');
$block20count = theme_space_get_setting('slidercount');
$block20class = theme_space_get_setting('block20class');
$herologotxt = theme_space_get_setting('herologotxt');
$herologo = $PAGE->theme->setting_file_url("herologo", "herologo");

if (theme_space_get_setting('showblock20sliderwrapper') == '1') {
...
for ($i = 1; $i <= $block20count; $i++) {
    ...
    $title = format_text(theme_space_get_setting("slidertitle" . $i), FORMAT_HTML, array('noclean' => true));
    $subtitle = format_text(theme_space_get_setting("slidersubtitle" . $i), FORMAT_HTML, array('noclean' => true));
    $caption = format_text(theme_space_get_setting("slidercap" . $i), FORMAT_HTML, array('noclean' => true));
    $img = $PAGE->theme->setting_file_url("sliderimage" . $i, "sliderimage" . $i);
    $url = theme_space_get_setting("sliderurl" . $i);
    $slideheight = theme_space_get_setting("slideheight" . $i);
    $sliderhtml = theme_space_get_setting("sliderhtml" . $i);
    $slidebackdrop = theme_space_get_setting("slidebackdrop" . $i);
...
if (theme_space_get_setting('sliderloop') == '1') {
    echo 'loop: true,';
}
if (theme_space_get_setting('sliderintervalenabled') == '1') {
